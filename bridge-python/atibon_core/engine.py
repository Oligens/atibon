...
class AtibonEngine:
    ...
    def ced_decide(self, samples: list[dict], thresholds: dict | None = None, previous_forensic_hash: str = "genesis") -> dict:
        """Run the multidimensional CED matrix without directly enforcing a rule."""
        return json.loads(_native.ced_decide(
            json.dumps(samples, separators=(",", ":")),
            json.dumps(thresholds or {}, separators=(",", ":")),
            previous_forensic_hash,
        ))

    def marl_simulate(self, seed: bytes, prior_risk: float = 0.5) -> dict:
        """Run one bounded Red/Blue co-evolution episode against synthetic state only."""
        return json.loads(_native.marl_simulate(seed, prior_risk))

    def semantic_mtd(self, seed: bytes, version: int, fields: list[str]) -> dict:
        """Generate a deterministic semantic variant and its compatibility aliases."""
        return json.loads(_native.semantic_mtd(seed, version, json.dumps(fields, separators=(",", ":"))))

    def optimize_defensive_topology(self, seed: bytes, iterations: int = 256) -> dict:
        """Optimize a defensive topology candidate; deployment remains quorum-gated."""
        return json.loads(_native.optimize_defensive_topology(seed, max(1, iterations)))
    ...
    def validate_barrier(self, envelope: dict, now_ms: int, current_epoch: int, current_version: int) -> dict:
        return json.loads(_native.validate_barrier(
    ...
    def commit_and_seal_barrier(self, policy: dict, sender_node_id: str, recipient_node_id: str, key_id: str, recipient_kem_public_key_hex: str, approvals: int, issued_at_ms: int) -> dict:
        ...
        envelope_json = _native.seal_barrier(
            json.dumps(candidate, separators=(",", ":")), sender_node_id, recipient_node_id,
            key_id, recipient_kem_public_key_hex, self.consensus.state_hash(), issued_at_ms,
        )
        self._last_policy_version = version
        return json.loads(envelope_json)

    def open_barrier(self, envelope: dict, recipient_kem_private_key_hex: str, trusted_signer_public_key_hex: str, now_ms: int, current_epoch: int, current_version: int) -> dict:
        """Verify, decapsulate and authenticate a received barrier before acceptance."""
        return json.loads(_native.open_barrier(
            json.dumps(envelope, separators=(",", ":")), recipient_kem_private_key_hex,
            trusted_signer_public_key_hex, now_ms, current_epoch, current_version,
        ))
