...
fn ced_mode(args: &[String]) -> ExitCode {
    ...
    // Never synthesize an unrestricted firewall rule. Enforcement is scoped to an
    // administrator/network-observed source IP supplied separately from telemetry.
    ...
    if args.iter().any(|a| a == "--ced-dry-run") {
        println!("ATIBON CED: would quarantine {ip} (automation_score={:.3})", observation.assessment.automation_score);
...
fn main() -> ExitCode {
    ...
    if let Some(pos) = args.iter().position(|a| a == "--block-ip") {
        ...
        match run_nft(&["add", "element", "inet", "atibon", "ssh_abuse", "{", &ip.to_string(), "timeout", "1h", "}"]) {
            ...
            Err(e) => { eprintln!("ATIBON: unable to block {ip}: {e}"); return ExitCode::from(1); }
        }
        return ExitCode::SUCCESS;
    }
