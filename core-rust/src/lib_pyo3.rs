...
fn open_barrier(envelope_json: &str, recipient_kem_private_key_hex: &str, trusted_signer_public_key_hex: &str, now_ms: u64, current_epoch: u64, current_version: u64) -> PyResult<String> {
    ...
    let result = crypto::transport::open_barrier(&envelope, recipient_kem_private_key_hex, trusted_signer_public_key_hex, now_ms, current_epoch, current_version)
        .map_err(pyo3::exceptions::PyValueError::new_err)?;
    serde_json::to_string(&result).map_err(pyo3::exceptions::PyValueError::new_err)
}

#[pyfunction]
fn matrix_gateway_process(packet: &[u8]) -> PyResult<String> {
    matrix_gateway::process(packet).and_then(|v| serde_json::to_string(&v).map_err(|e| e.to_string()))
...
fn egress_privacy_decide(destination: &str, reputation_score: u8, request_count: u64, policy_json: &str) -> PyResult<String> {
    let policy: egress_privacy::EgressPolicy = serde_json::from_str(policy_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    serde_json::to_string(&egress_privacy::decide(destination, reputation_score, request_count, &policy))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
fn egress_pipeline_decide(destination: &str, reputation_score: u8, request_count: u64, policy_json: &str, runtime_mode: &str, audit_path: &str) -> PyResult<String> {
    ...
    let mode = match runtime_mode {
        ...
        other => return Err(pyo3::exceptions::PyValueError::new_err(format!("invalid egress runtime mode: {other}"))),
    };
...
fn list_defensive_agents() -> PyResult<String> {
    serde_json::to_string(agents::descriptors()).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
/// Execute all learning gates in native code. The poisoning guard result and
/// approval are explicit inputs to the final promotion gate; neither an event
/// nor feedback can directly mutate active policy.
#[pyfunction]
fn governed_learning_cycle(
    event_json: &str,
    classification_json: &str,
    decision_json: &str,
    result_json: &str,
    feedback_json: &str,
    shadow_samples_json: &str,
    active_policy_version: u64,
    poisoning_guard_ok: bool,
    human_or_automatic_approval: bool,
) -> PyResult<String> {
    let event: learning::LearningEvent = serde_json::from_str(event_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    let classification: learning::Classification = serde_json::from_str(classification_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    let decision: learning::Decision = serde_json::from_str(decision_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    let result: learning::ResultObservation = serde_json::from_str(result_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    let feedback: learning::Feedback = serde_json::from_str(feedback_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
    let samples: Vec<(f64, bool)> = serde_json::from_str(shadow_samples_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;

    let mut state = learning::LearningState { active_policy_version, ..Default::default() };
    state.observe(event.anomaly_score, result.blocked, feedback.false_positive, &event.source_digest);
    let candidate = state
        .propose_candidate(&classification, &decision, &result, &feedback, &format!("candidate-{}", event.event_id))
        .map_err(pyo3::exceptions::PyValueError::new_err)?;
    let validation = state.validate_candidate(&candidate);
    let shadow = state.shadow_validate(&candidate, &samples);
    let promotion = state.promote(&candidate, &validation, &shadow, poisoning_guard_ok, human_or_automatic_approval);
    let cycle = learning::LearningCycle { event, classification, decision, result, feedback, candidate, validation, shadow, promotion };
    serde_json::to_string(&cycle).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
}
...
fn native_module(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(governed_learning_cycle, m)?)?;
