...
impl LearningState {
    ...
    pub fn propose_candidate(
        ...
        Ok(PolicyCandidate {
            ...
            reason: format!(
                "governed feedback: {} / {}",
                classification.label, result.outcome
            ),
    ...
    pub fn shadow_validate(
        ...
        for (score, expected_block) in samples {
            if (*score >= candidate.threshold) != *expected_block {
    ...
    pub fn promote(
        ...
        let validation_ok =
            validation.integrity_ok && validation.safety_ok && validation.compatible;
...
mod tests {
    ...
    fn promotion_requires_all_gates() {
        ...
        assert!(
            !state
                .promote(&candidate, &validation, &shadow, false, true)
                .promoted
        );
        assert_eq!(state.active_policy_version, 0);
        assert!(
            state
                .promote(&candidate, &validation, &shadow, true, true)
                .promoted
        );
