...
fn default_layer_window_ms() -> u64 {
    DEFAULT_LAYER_WINDOW_MS
}
fn default_fuzz_similarity() -> f64 {
    DEFAULT_FUZZ_SIMILARITY
}
...
fn quantize_delta(delta: u64) -> u64 {
    delta.saturating_div(10).min(10_000)
}
...
pub fn assess(samples: &[TelemetrySample]) -> BehavioralAssessment {
    if samples.len() < 3 {
        return BehavioralAssessment {
            ...
            suspicious: false,
            vector: vec![],
            reasons: vec!["insufficient_telemetry".into()],
        };
    }
    let mut deltas = Vec::with_capacity(samples.len() - 1);
    let mut sizes = Vec::with_capacity(samples.len());
    for pair in samples.windows(2) {
        deltas.push(quantize_delta(pair[1].at_ms.saturating_sub(pair[0].at_ms)));
    }
    for s in samples {
        sizes.push((s.bytes as u64).min(1_048_576) / 64);
    }
    ...
    let unique_paths = samples
        ...
        .collect::<HashSet<_>>()
        .len();
    let unique_mutations = samples
        .iter()
        .map(|s| s.mutation.as_str())
        .collect::<HashSet<_>>()
        .len();
    ...
    let automation_score =
        (0.35 * low_timing_entropy + 0.35 * path_repetition + 0.30 * mutation_repetition)
            .clamp(0.0, 1.0);
    let suspicious = automation_score >= 0.68;
    let mut reasons = Vec::new();
    if low_timing_entropy > 0.55 {
        reasons.push("low_kinematic_entropy".into());
    }
    if path_repetition > 0.55 {
        reasons.push("repetitive_exploration_loop".into());
    }
    if mutation_repetition > 0.55 {
        reasons.push("repetitive_payload_mutation".into());
    }
...
fn payload_similarity(a: &str, b: &str) -> f64 {
    ...
    for w in aa.windows(3).take(4096) {
        if w.len() == 3 {
            sa.insert([w[0], w[1], w[2]]);
        }
    }
    for w in bb.windows(3).take(4096) {
        if w.len() == 3 {
            sb.insert([w[0], w[1], w[2]]);
        }
    }
...
fn layer_velocity(samples: &[TelemetrySample], cfg: &ThreatThresholdConfig) -> (f64, u8, bool) {
    ...
    if max_layer < cfg.critical_layer_count {
        return (0.0, max_layer, false);
    }
    let first = samples
        .iter()
        .filter(|s| s.layer > 0)
        ...
        .min()
        .unwrap_or(0);
    let target_time = samples
        .iter()
        .filter(|s| s.layer >= cfg.critical_layer_count)
        .map(|s| s.at_ms)
        .min()
        .unwrap_or(first);
    ...
    let velocity = if elapsed == 0 {
        ...
        (max_layer as f64 - 1.0) / (elapsed as f64 / 1000.0)
    };
...
fn stable_id(bytes: &[u8]) -> String {
    hex_id(&Sha256::digest(bytes))
}

pub fn decide(
    samples: &[TelemetrySample],
    cfg: &ThreatThresholdConfig,
    ...
    let fuzz_triggered =
        convergence >= cfg.fuzz_similarity_threshold && assessment.automation_score >= 0.55;
    let pqc_triggered = pqc_violation(samples);

    let mut signals = Vec::new();
    let mut reasons = assessment.reasons.clone();
    if layer_triggered {
        signals.push("layer_penetration_velocity".into());
        reasons.push("three_or_more_layers_in_critical_window".into());
    }
    if fuzz_triggered {
        signals.push("stochastic_fuzz_convergence".into());
        reasons.push("high_payload_similarity_with_low_behavioral_entropy".into());
    }
    if pqc_triggered {
        signals.push("pqc_integrity_violation".into());
        reasons.push("trusted_crypto_subsystem_reported_integrity_violation".into());
    }
    if assessment.suspicious {
        signals.push("automation_behavior".into());
    }
    ...
    let critical = pqc_triggered
        || (layer_triggered && fuzz_triggered)
        || cumulative_score >= cfg.critical_score;
    ...
    let containment = ContainmentPlan {
        mode: if permanent {
            ContainmentMode::Permanent
        } else if elevated {
            ContainmentMode::Adaptive
        } else {
            ContainmentMode::Monitor
        },
    ...
    let mut record_material = serde_json::to_vec(&(
...
pub fn deceptive_graph(assessment: &BehavioralAssessment) -> Vec<DecoyNode> {
    ...
    vec![
        DecoyNode {
            id: format!("entry-{salt}"),
            kind: "api-mirror".into(),
            response_shape: "stable-but-fake-schema".into(),
            next: vec![format!("probe-{salt}")],
        },
        DecoyNode {
            id: format!("probe-{salt}"),
            kind: "vulnerable-mirror".into(),
            response_shape: "synthetic-debug-surface".into(),
            next: vec![format!("sink-{salt}")],
        },
        DecoyNode {
            id: format!("sink-{salt}"),
            kind: "isolated-sink".into(),
            response_shape: "telemetry-only".into(),
            next: vec![],
        },
    ]
}
...
pub fn mutate_rule(assessment: &BehavioralAssessment) -> Rule {
    ...
    let action = if assessment.automation_score >= 0.85 {
        Action::Reject
    } else {
        Action::Drop
    };
    Rule {
        name: format!("ced-{}", &digest[..16]),
        protocol: None,
        dst_port: None,
        action,
    }
...
pub fn observe(samples: &[TelemetrySample]) -> AttackObservation {
    ...
    AttackObservation {
        ...
        generated_rule: rule,
        rule_digest,
        signature_algorithm: "ML-DSA-65 (FIPS 204)".into(),
    }
}
...
mod tests {
    ...
    fn detects_repetitive_automation() {
        let samples = (0..12)
            .map(|i| sample(i * 100, 1, "same-payload"))
            .collect::<Vec<_>>();
    ...
    fn mutation_is_fail_closed() {
        let samples = (0..4)
            .map(|i| sample(i * 10, 1, "same"))
            .collect::<Vec<_>>();
        assert!(matches!(
            mutate_rule(&assess(&samples)).action,
    ...
    fn high_payload_convergence_is_a_signal_not_identity_proof() {
        let samples = vec![
            sample(0, 1, "AAAAAA-payload-stable"),
            sample(100, 1, "AAAAAA-payload-stable"),
            sample(200, 1, "AAAAAA-payload-stable"),
        ];
        let d = decide(&samples, &ThreatThresholdConfig::default(), "genesis");
    ...
    fn pqc_violation_forces_critical_containment() {
        ...
        let d = decide(
            &[s, sample(10, 1, "y"), sample(20, 1, "z")],
            &ThreatThresholdConfig::default(),
            "genesis",
        );
    ...
    fn cumulative_matrix_creates_permanent_containment() {
        let samples = vec![
            sample(0, 1, "stable-payload-001"),
            sample(100, 2, "stable-payload-001"),
            sample(500, 3, "stable-payload-001"),
        ];
        let d = decide(&samples, &ThreatThresholdConfig::default(), "genesis");
