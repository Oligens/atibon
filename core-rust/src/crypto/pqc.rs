...
impl PqcFacade {
    ...
    pub fn backend(&self) -> &'static str {
        if cfg!(feature = "pqc-native") {
            "rustcrypto-fips203-fips204"
        } else {
            "unconfigured"
        }
    }
    pub fn encapsulation_required(&self) -> bool {
        true
    }
    ...
    pub fn sign_barrier(&self, digest_hex: &str) -> PyResult<String> {
        #[cfg(feature = "pqc-native")]
        {
            ...
            let seed_hex = std::env::var("ATIBON_MLDSA65_SEED_HEX").map_err(|_| {
                pyo3::exceptions::PyRuntimeError::new_err(
                    "ATIBON_MLDSA65_SEED_HEX is not configured",
                )
            })?;
            let seed = decode_hex(&seed_hex).map_err(pyo3::exceptions::PyValueError::new_err)?;
            if seed.len() != ML_DSA65_SEED_BYTES {
                return Err(pyo3::exceptions::PyValueError::new_err(
                    "ML-DSA-65 seed must be exactly 32 bytes",
                ));
            }
            let seed_array: [u8; ML_DSA65_SEED_BYTES] =
                seed.as_slice().try_into().map_err(|_| {
                    pyo3::exceptions::PyValueError::new_err("invalid ML-DSA seed length")
                })?;
        ...
        #[cfg(not(feature = "pqc-native"))]
        {
            ...
            Err(pyo3::exceptions::PyRuntimeError::new_err(
                "native PQC backend is disabled; build with --features pqc-native",
            ))
        }
    }
    ...
    pub fn verify_barrier(
        ...
        #[cfg(feature = "pqc-native")]
        {
            ...
            let sig_bytes =
                decode_hex(signature_hex).map_err(pyo3::exceptions::PyValueError::new_err)?;
            let pk_array: [u8; ML_DSA65_PUBLIC_KEY_BYTES] =
                pk.as_slice().try_into().map_err(|_| {
                    pyo3::exceptions::PyValueError::new_err(
                        "ML-DSA-65 public key must be exactly 1952 bytes",
                    )
                })?;
            let sig_array: [u8; ML_DSA65_SIGNATURE_BYTES] =
            ...
            let sig = ml_dsa::Signature::<MlDsa65>::decode(&encoded_sig).ok_or_else(|| {
                pyo3::exceptions::PyValueError::new_err("invalid ML-DSA signature encoding")
            })?;
        ...
        #[cfg(not(feature = "pqc-native"))]
        {
            ...
            Err(pyo3::exceptions::PyRuntimeError::new_err(
                "native PQC backend is disabled",
            ))
        }
    }
...
fn decode_hex(input: &str) -> Result<Vec<u8>, String> {
    if !input.is_ascii() {
        return Err("hex input must contain only ASCII hexadecimal characters".into());
    }
...
mod tests {
    use super::*;
    #[test]
    fn hex_round_trip_is_canonical_lowercase() {
        let bytes = [0x00, 0xAB, 0xFF, 0x10];
        assert_eq!(hex(&bytes), "00abff10");
        assert_eq!(decode_hex("00ABff10").unwrap(), bytes);
    }
    #[test]
    ...
    #[test]
    fn fips203_204_size_constants_match_selected_parameter_sets() {
        assert_eq!(ML_DSA65_SEED_BYTES, 32);
        assert_eq!(ML_DSA65_PUBLIC_KEY_BYTES, 1952);
        assert_eq!(ML_DSA65_SIGNATURE_BYTES, 3309);
    }
    #[cfg(feature = "pqc-native")]
    #[test]
    fn ml_dsa65_signature_round_trip_and_tamper_rejection() {
        ...
        assert_eq!(
            AsRef::<[u8]>::as_ref(&encoded_sig).len(),
            ML_DSA65_SIGNATURE_BYTES
        );
    }
    #[cfg(feature = "pqc-native")]
    #[test]
    fn ml_kem768_round_trip_uses_fips203_sizes() {
        ...
        assert_eq!(
            AsRef::<[u8]>::as_ref(&ciphertext).len(),
            1088
        );
        assert_eq!(
            AsRef::<[u8]>::as_ref(&sender_secret).len(),
            32
        );
    }
}
